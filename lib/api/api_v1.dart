...
class ApiV1 {
  ...
  login(username, password) async {
    ...
    final response = await http.post(actionUrl, headers: {
  ...
  login_by_qr(login_code) async {
    Uri actionUrl = parseUri(appDomain(), '/api/v1/get_token_from_login_code');
    final response = await http.post(actionUrl, headers: {
      'Accept': 'application/json'
    }, body: {
  ...
  checkToken(token) async {
    Uri actionUrl = parseUri(appDomain(), '/api/v1/check_token');

    final response = await http.post(actionUrl, headers: {'Accept': 'application/json'}, body: {"token": "$token"});

    if (response.statusCode != 200) {
      showVDialog(title: 'Internal Server Error', text: 'Internal Server Error');
      return null;
    }
  ...
  getUserEmployeeDetail(token) async {
    Uri actionUrl = parseUri(appDomain(), '/api/v1/me');
  ...
  clockInOut({token, gps, address, time_in_type, type, date}) async {
    ...
    aLog({"token": "$token", "gps_lat_lng": "$gps", "address": "$address", "type": "$type", "time_in_type": "$time_in_type", "date": "$date"});

    if (time_in_type == null || time_in_type == 0) {
      showVDialog(title: 'Error', text: 'time_in_type', type: 'error');
      return;
    }
    final response = await http.post(actionUrl,
        headers: {'Accept': 'application/json'},
        body: {"token": "$token", "gps_lat_lng": "$gps", "address": "$address", "type": "$type", "time_in_type": "$time_in_type", "date": "$date"});
  ...
  getYesterdayPendingTimesheet({required token}) async {
    Uri actionUrl = parseUri(appDomain(), '/api/v1/get_yesterday_pending_timesheet');
  ...
  Future<bool> validateOldPassword({required token, String oldPass = ''}) async {
    bool isPasswordValid = false;
    Uri actionUrl = parseUri(appDomain(), '/api/v1/preference/validate_old_password');
    final response = await http.post(actionUrl, headers: {
      'Accept': 'application/json'
    }, body: {
      "token": "$token",
  ...
  getTimesheetTrans({token, page, perPage}) async {
    Uri actionUrl = parseUri(appDomain(), '/api/v1/timesheet/get_timesheet_trans');
    final response = await http.post(actionUrl, headers: {
      'Accept': 'application/json'
    }, body: {
      "token": "$token",
  ...
  logLocation({token, address, coordinate, device_info}) async {
    ...
    final response = await http.post(actionUrl,
        headers: {'Accept': 'application/json'},
        body: {"token": "$token", "address": "$address", "coordinate": "$coordinate", "device_info": device_info});
